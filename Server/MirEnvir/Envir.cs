...
namespace Server.MirEnvir
{
    ...
    public class Envir
    {
        ...
        public const string RaidDBPath = BackUpPath + @"Raids\";
        ...
        public static int HeroLoadVersion = 112;
        ...
        #region MySQL
        public MirMySQL sql;
        public bool UpdatedSQLRank = false;
        #endregion
        ...
        public string Cycles
        {
            ...
            {
                string tmp = "";
                if (Settings.Multithreaded && (SMain.Envir.MobThreads != null))
                {
                    tmp = string.Format("CycleDelays: {0:0000}", Envir.LastRunTime);
                    for (int i = 0; i < SMain.Envir.MobThreads.Length; i++)
                    {
                        tmp = tmp + string.Format("|{0:0000}", SMain.Envir.MobThreads[i].LastRunTime);

                    }
                }
                else
                    tmp = string.Format("CycleDelay: {0}", Envir.LastRunTime); ;
                return tmp;
            }
        }
        public string Uptime
        {
            get
            {
               return $"Running Time: {SMain.Envir.Stopwatch.ElapsedMilliseconds / 1000 / 60 / 60 / 24}d:{SMain.Envir.Stopwatch.ElapsedMilliseconds / 1000 / 60 / 60}h:{SMain.Envir.Stopwatch.ElapsedMilliseconds / 1000 / 60}m:{SMain.Envir.Stopwatch.ElapsedMilliseconds / 1000 % 60}s";
            } 
        }
        
        public RandomProvider Random = new RandomProvider();

        public DateTime _DatFileTime;
        public List<ItemInfo> _DatFileInfos = new List<ItemInfo>();
        ...
        public bool IsServerManager { get { return Settings.UseServerManager; } }
        public List<ServerManager> ServerManagers = new List<ServerManager>();
        private TcpListener _ServerManager;
        public List<string> ManagerCommands = new List<string>();
        #endregion
        //Server DB
        public int MapIndex, ItemIndex, MonsterIndex, NPCIndex, QuestIndex, GameshopIndex, ConquestIndex, RespawnIndex, RecipeIndex, MapEventIndex, LMS_BR_Index, RaidIndex;
        public byte CraftIndex;
        ...
        public List<RaidMap_Info> Raids = new List<RaidMap_Info>();

        public List<RecipeShop> RecipeShopContents = new List<RecipeShop>();

        public List<int> RandomQuestsID = new List<int>();
        ...
        public List<AuctionHouseInfo> AHouseList = new List<AuctionHouseInfo>();
        ...
        public List<DropInfo> RuneDrops = new List<DropInfo>();
        public List<DropInfo> FortuneBoxWeaponDrops = new List<DropInfo>();
        public List<DropInfo> FortuneBoxArmourDrops = new List<DropInfo>();
        public List<DropInfo> FortuneBoxAccessoryDrops = new List<DropInfo>();
        public List<DropInfo> FortuneBoxOrbDrops = new List<DropInfo>();
        ...
        public List<RobotInfo> RobotCommandList = new List<RobotInfo>();
        ...
        public List<AI_Customiser> CustomAIs = new List<AI_Customiser>();
        public List<CraftItem> CraftItems = new List<CraftItem>();

        public List<string> LineMessages = new List<string>();

        public HeroConfig heroConfig = new HeroConfig();

        public List<HeroRank> HeroRankings = new List<HeroRank>();
        ...
        public static bool KillKey1 = false;
        public static bool KillKey2 = false;

        private long warTime, mailTime, guildTime, conquestTime, robotTime , questTime, viptime, GuildTime, runtimeValuesTime;
        private int DailyTime = DateTime.Now.Day;

        public bool MikeSource = false;
        ...
        public bool Contains(string source, string cont, StringComparison comparison)
        {
            return source.IndexOf(cont, comparison) >= 0;
        }
        ...
        public void LoadLogNotice()
        {
            ...
            for (int i = 0; i < contents.Length; i++)
            {
                ...
                //if (Contains(contents[i], "LINK", StringComparison.OrdinalIgnoreCase))
                {
                    string[] splitStr = contents[i].Split('=');
                    temp.Link = splitStr[1];
                }
                else if (contents[i].Contains("TITLE") &&
                    string.Compare(contents[i], "TITLE", false) > 0)
                //else if (Contains(contents[i], "TITLE", StringComparison.OrdinalIgnoreCase))
                {
                    string[] splitStr = contents[i].Split('=');
            ...
            for (int i = 0; i < MagicInfoList.Count; i++)
            {
                switch(MagicInfoList[i].Spell)
                {
                    ...
                    case Spell.BlazingSword:
                        MagicInfoList[i].MultiplierBase = 1.8f;
                        MagicInfoList[i].MultiplierBonus = 0.4f;
                        break;
        ...
        private void FillMagicInfoList()
        {
            ...
            if (!MagicExists(Spell.SliceNDice)) MagicInfoList.Add(new MagicInfo { Name = "SliceNDice", Spell = Spell.SliceNDice, Icon = 80, Level1 = 50, Level2 = 53, Level3 = 55, Need1 = 1000, Need2 = 1500, Need3 = 3000, BaseCost = 30, LevelCost = 5, Range = 0, MultiplierBase = 1.4f, MultiplierBonus = 0.4f });
            if (!MagicExists(Spell.BrokenSoulCut)) MagicInfoList.Add(new MagicInfo { Name = "BrokenSoulCut", Spell = Spell.BrokenSoulCut, Icon = 80, Level1 = 50, Level2 = 53, Level3 = 55, Need1 = 1000, Need2 = 1500, Need3 = 3000, BaseCost = 30, LevelCost = 5, Range = 0, MultiplierBase = 1.4f, MultiplierBonus = 0.4f });
            if (!MagicExists(Spell.BrokenSoul)) MagicInfoList.Add(new MagicInfo { Name = "BrokenSoul", Spell = Spell.BrokenSoul, Icon = 80, Level1 = 50, Level2 = 53, Level3 = 55, Need1 = 1000, Need2 = 1500, Need3 = 3000, BaseCost = 30, LevelCost = 5, Range = 0, MultiplierBase = 1.4f, MultiplierBonus = 0.4f });
            ...
            if (!MagicExists(Spell.FireSpray)) MagicInfoList.Add(new MagicInfo { Name = "FireSpray", Spell = Spell.FireSpray, Icon = 0, Level1 = 7, Level2 = 9, Level3 = 11, Need1 = 200, Need2 = 350, Need3 = 700, BaseCost = 3, LevelCost = 2, MPowerBase = 8, PowerBase = 2, Range = 9 });
            ...
            if (!MagicExists(Spell.FastMove)) MagicInfoList.Add(new MagicInfo { Name = "FastMove", Spell = Spell.FastMove, Icon = 23, Level1 = 60, Level2 = 61, Level3 = 62, Need1 = 2200, Need2 = 3300, Need3 = 4400, BaseCost = 65, LevelCost = 8, MPowerBase = 12, PowerBase = 4, Range = 9 });
            ...
            if (!MagicExists(Spell.DragonFlames)) MagicInfoList.Add(new MagicInfo { Name = "DragonFlames", Spell = Spell.DragonFlames, Icon = 32, Level1 = 35, Level2 = 37, Level3 = 40, Need1 = 4000, Need2 = 8000, Need3 = 12000, BaseCost = 33, LevelCost = 3, MPowerBase = 12, PowerBase = 14, Range = 9 });
            if (!MagicExists(Spell.ThunderClap)) MagicInfoList.Add(new MagicInfo { Name = "ThunderClap", Spell = Spell.ThunderClap, Icon = 32, Level1 = 35, Level2 = 37, Level3 = 40, Need1 = 4000, Need2 = 8000, Need3 = 12000, BaseCost = 33, LevelCost = 3, MPowerBase = 12, PowerBase = 14, Range = 9 });
            if (!MagicExists(Spell.ChopChopStar)) MagicInfoList.Add(new MagicInfo { Name = "ChopChopStar", Spell = Spell.ChopChopStar, Icon = 32, Level1 = 35, Level2 = 37, Level3 = 40, Need1 = 4000, Need2 = 8000, Need3 = 12000, BaseCost = 33, LevelCost = 3, MPowerBase = 12, PowerBase = 14, Range = 9 });
            if (!MagicExists(Spell.SoulEaterSwamp)) MagicInfoList.Add(new MagicInfo { Name = "SoulEaterSwamp", Spell = Spell.SoulEaterSwamp, Icon = 32, Level1 = 35, Level2 = 37, Level3 = 40, Need1 = 4000, Need2 = 8000, Need3 = 12000, BaseCost = 33, LevelCost = 3, MPowerBase = 12, PowerBase = 14, Range = 9 });
            if (!MagicExists(Spell.HandOfGod)) MagicInfoList.Add(new MagicInfo { Name = "HandOfGod", Spell = Spell.HandOfGod, Icon = 32, Level1 = 35, Level2 = 37, Level3 = 40, Need1 = 4000, Need2 = 8000, Need3 = 12000, BaseCost = 33, LevelCost = 3, MPowerBase = 12, PowerBase = 14, Range = 9 });
            if (!MagicExists(Spell.ShadowStep)) MagicInfoList.Add(new MagicInfo { Name = "ShadowStep", Spell = Spell.ShadowStep, Icon = 23, Level1 = 60, Level2 = 61, Level3 = 62, Need1 = 2200, Need2 = 3300, Need3 = 4400, BaseCost = 65, LevelCost = 8, MPowerBase = 12, PowerBase = 4, Range = 9 });
            if (!MagicExists(Spell.DragonTears)) MagicInfoList.Add(new MagicInfo { Name = "DragonTears", Spell = Spell.DragonTears, Icon = 32, Level1 = 35, Level2 = 37, Level3 = 40, Need1 = 4000, Need2 = 8000, Need3 = 12000, BaseCost = 33, LevelCost = 3, MPowerBase = 12, PowerBase = 14, Range = 9 });
            if (!MagicExists(Spell.ThunderRolls)) MagicInfoList.Add(new MagicInfo { Name = "ThunderRolls", Spell = Spell.ThunderRolls, Icon = 32, Level1 = 35, Level2 = 37, Level3 = 40, Need1 = 4000, Need2 = 8000, Need3 = 12000, BaseCost = 33, LevelCost = 3, MPowerBase = 12, PowerBase = 14, Range = 9 });
            if (!MagicExists(Spell.StarPower)) MagicInfoList.Add(new MagicInfo { Name = "StarPower", Spell = Spell.StarPower, Icon = 32, Level1 = 35, Level2 = 37, Level3 = 40, Need1 = 4000, Need2 = 8000, Need3 = 12000, BaseCost = 33, LevelCost = 3, MPowerBase = 12, PowerBase = 14, Range = 9 });
            if (!MagicExists(Spell.SoulEater)) MagicInfoList.Add(new MagicInfo { Name = "SoulEater", Spell = Spell.SoulEater, Icon = 32, Level1 = 35, Level2 = 37, Level3 = 40, Need1 = 4000, Need2 = 8000, Need3 = 12000, BaseCost = 33, LevelCost = 3, MPowerBase = 12, PowerBase = 14, Range = 9 });
            if (!MagicExists(Spell.GodsHand)) MagicInfoList.Add(new MagicInfo { Name = "GodsHand", Spell = Spell.GodsHand, Icon = 32, Level1 = 35, Level2 = 37, Level3 = 40, Need1 = 4000, Need2 = 8000, Need3 = 12000, BaseCost = 33, LevelCost = 3, MPowerBase = 12, PowerBase = 14, Range = 9 });
            ...
            if (!MagicExists(Spell.HealingCircle)) MagicInfoList.Add(new MagicInfo { Name = "HealingCircle", Spell = Spell.HealingCircle, Icon = 82, Level1 = 10, Level2 = 10, Level3 = 10, Need1 = 10, Need2 = 11, Need3 = 12, BaseCost = 30, LevelCost = 40, DelayBase = 3800, DelayReduction = 0 });
            if (!MagicExists(Spell.LastJudgement)) MagicInfoList.Add(new MagicInfo { Name = "LastJudgement", Spell = Spell.LastJudgement, Icon = 32, Level1 = 35, Level2 = 37, Level3 = 40, Need1 = 4000, Need2 = 8000, Need3 = 12000, BaseCost = 33, LevelCost = 3, MPowerBase = 12, PowerBase = 14, Range = 9 });
            if (!MagicExists(Spell.Judgement)) MagicInfoList.Add(new MagicInfo { Name = "Judgement", Spell = Spell.Judgement, Icon = 32, Level1 = 35, Level2 = 37, Level3 = 40, Need1 = 4000, Need2 = 8000, Need3 = 12000, BaseCost = 33, LevelCost = 3, MPowerBase = 12, PowerBase = 14, Range = 9 });
            ...
            if (!MagicExists(Spell.Haste)) MagicInfoList.Add(new MagicInfo { Name = "Haste", Spell = Spell.Haste, Icon = 60, Level1 = 15, Level2 = 20, Level3 = 26, Need1 = 2000, Need2 = 3000, Need3 = 6000, BaseCost = 3, LevelCost = 2, Range = 0 });
            ...
            if (!MagicExists(Spell.MoonMist)) MagicInfoList.Add(new MagicInfo { Name = "MoonMist", Spell = Spell.MoonMist, Icon = 82, Level1 = 50, Level2 = 53, Level3 = 56, Need1 = 12000, Need2 = 16000, Need3 = 24000, BaseCost = 19, LevelCost = 5, });
            if (!MagicExists(Spell.SoulReaper)) MagicInfoList.Add(new MagicInfo { Name = "SoulReaper", Spell = Spell.SoulReaper, Icon = 32, Level1 = 35, Level2 = 37, Level3 = 40, Need1 = 4000, Need2 = 8000, Need3 = 12000, BaseCost = 33, LevelCost = 3, MPowerBase = 12, PowerBase = 14, Range = 9 });
            if (!MagicExists(Spell.SoulStealer)) MagicInfoList.Add(new MagicInfo { Name = "SoulStealer", Spell = Spell.SoulStealer, Icon = 32, Level1 = 35, Level2 = 37, Level3 = 40, Need1 = 4000, Need2 = 8000, Need3 = 12000, BaseCost = 33, LevelCost = 3, MPowerBase = 12, PowerBase = 14, Range = 9 });
            ...
            if (!MagicExists(Spell.FuryWaves)) MagicInfoList.Add(new MagicInfo { Name = "FuryWaves", Spell = Spell.FuryWaves, Icon = 71, Level1 = 50, Level2 = 53, Level3 = 56, Need1 = 12000, Need2 = 16000, Need3 = 24000, BaseCost = 19, LevelCost = 5, Range = 0 });
            if (!MagicExists(Spell.FrozenRains)) MagicInfoList.Add(new MagicInfo { Name = "FrozenRains", Spell = Spell.FrozenRains, Icon = 51, Level1 = 44, Level2 = 47, Level3 = 50, Need1 = 8000, Need2 = 16000, Need3 = 24000, BaseCost = 65, LevelCost = 10, MPowerBase = 30, MPowerBonus = 10, PowerBase = 20, PowerBonus = 5, Range = 9 });
            if (!MagicExists(Spell.HeadShot)) MagicInfoList.Add(new MagicInfo { Name = "HeadShot", Spell = Spell.HeadShot, Icon = 12, Level1 = 18, Level2 = 21, Level3 = 24, Need1 = 1300, Need2 = 2700, Need3 = 4000, BaseCost = 3, LevelCost = 1, MPowerBase = 8, PowerBase = 3, Range = 9 });
            if (!MagicExists(Spell.LavaKing)) MagicInfoList.Add(new MagicInfo { Name = "LavaKing", Spell = Spell.LavaKing, Icon = 44, Level1 = 42, Level2 = 43, Level3 = 45, Need1 = 6000, Need2 = 11000, Need3 = 16000, BaseCost = 45, LevelCost = 8, MPowerBase = 100, PowerBase = 25, Range = 9 });
            if (!MagicExists(Spell.BladeStorm)) MagicInfoList.Add(new MagicInfo { Name = "BladeStorm", Spell = Spell.BladeStorm, Icon = 44, Level1 = 42, Level2 = 43, Level3 = 45, Need1 = 6000, Need2 = 11000, Need3 = 16000, BaseCost = 45, LevelCost = 8, MPowerBase = 100, PowerBase = 25, Range = 9 });
            if (!MagicExists(Spell.DragNet)) MagicInfoList.Add(new MagicInfo { Name = "DragNet", Spell = Spell.DragNet, Icon = 44, Level1 = 42, Level2 = 43, Level3 = 45, Need1 = 6000, Need2 = 11000, Need3 = 16000, BaseCost = 45, LevelCost = 8, MPowerBase = 100, PowerBase = 25, Range = 0 });
            if (!MagicExists(Spell.YinYangTiger)) MagicInfoList.Add(new MagicInfo { Name = "YinYangTiger", Spell = Spell.YinYangTiger, Icon = 44, Level1 = 42, Level2 = 43, Level3 = 45, Need1 = 6000, Need2 = 11000, Need3 = 16000, BaseCost = 45, LevelCost = 8, MPowerBase = 100, PowerBase = 25, Range = 0 });
            if (!MagicExists(Spell.HolyShield)) MagicInfoList.Add(new MagicInfo { Name = "HolyShield", Spell = Spell.HolyShield, Icon = 104, Level1 = 60, Level2 = 70, Level3 = 72, Need1 = 2000, Need2 = 3500, Need3 = 7000, BaseCost = 2, LevelCost = 2, Range = 9 });
            if (!MagicExists(Spell.BlazingSword)) MagicInfoList.Add(new MagicInfo { Name = "BlazingSword", Spell = Spell.BlazingSword, Icon = 25, Level1 = 35, Level2 = 37, Level3 = 40, Need1 = 2000, Need2 = 4000, Need3 = 6000, BaseCost = 7, Range = 0, MultiplierBase = 1.4f, MultiplierBonus = 0.4f });
            if (!MagicExists(Spell.SummonHolyDragon)) MagicInfoList.Add(new MagicInfo { Name = "SummonHolyDragon", Spell = Spell.SummonHolyDragon, Icon = 40, Level1 = 38, Level2 = 41, Level3 = 43, Need1 = 4000, Need2 = 6000, Need3 = 9000, BaseCost = 28, LevelCost = 4, Range = 0 });
        }
        ...
        private string CanStartEnvir()
        {
            if (Settings.EnforceDBChecks)
            {
                ...
                if (GetMonsterInfo(Settings.SuperSkeletonName1, true) == null) return "Cannot start server without mob: " + Settings.SuperSkeletonName1;
                if (GetMonsterInfo(Settings.SuperSkeletonName2, true) == null) return "Cannot start server without mob: " + Settings.SuperSkeletonName2;
                if (GetMonsterInfo(Settings.SuperSkeletonName3, true) == null) return "Cannot start server without mob: " + Settings.SuperSkeletonName3;
                if (GetMonsterInfo(Settings.ShinsuName, true) == null) return "Cannot start server without mob: " + Settings.ShinsuName;
                if (GetMonsterInfo(Settings.SuperShinsuName, true) == null) return "Cannot start server without mob: " + Settings.SuperShinsuName;
                if (GetMonsterInfo(Settings.SuperShinsuName2, true) == null) return "Cannot start server without mob: " + Settings.SuperShinsuName2;
                if (GetMonsterInfo(Settings.SuperShinsuName4, true) == null) return "Cannot start server without mob: " + Settings.SuperShinsuName4;
                ...
                if (GetMonsterInfo(Settings.DragonName, true) == null) return "Cannot start server without mob: " + Settings.DragonName;
                if (GetMonsterInfo(Settings.DragonName1, true) == null) return "Cannot start server without mob: " + Settings.DragonName1;
            }
        ...
        private void ManagerLoop()
        {
            try
            {
                ...
                while (ManagerRunning)
                {
                    if (IsServerManager)
                    {
                        lock (ServerManagers)
                        {
                            for (int i = ServerManagers.Count - 1; i >= 0; i--)
                            {
                                if (ServerManagers[i] == null)
                                    continue;
                                ServerManagers[i].Process();
                                
                            }
                        }
                        if (ManagerCommands != null && 
                            ManagerCommands.Count > 0 &&
                            ServerManagers != null && 
                            ServerManagers.Count >= 1)
                        {
                            for (int i = ManagerCommands.Count - 1; i >= 0; i--)
                            {
                                ServerManagers[0].ProcessCommand(ManagerCommands[i]);
                                ManagerCommands.RemoveAt(i);
                            }
                        }
                    }
                }
            ...
            catch (Exception ex)
            {
                ...
                {
                    for (int i = ServerManagers.Count - 1; i >= 0; i--)
                        ServerManagers[i].SendDisconnect();
                }
        ...
        private void WorkLoop()
        {
            try
            {
                ...
                long StartTime = Time;
                long updateHeroRanks = Time + Settings.Minute * 5;
                int processCount = 0;
                    ...
                    while (Running)
                    {
                        ...
                        while ((!TheEnd) && (Stopwatch.ElapsedMilliseconds - Start < 20))
                        {
                            ...
                            else
                            {
                                ...
                                if (!Settings.Multithreaded || ((current.Value.Race != ObjectType.Monster || current.Value.Race != ObjectType.Hero) || (current.Value.Master != null)))
                                {
                        ...
                        if (Time >= lineMessageTime)
                        {
                            ...
                            if (LastLineMessage > LineMessages.Count - 1)
                                LastLineMessage = 0;
                            lineMessageTime = Time + Settings.Minute * 10;
                        }
                        ...
                        if (Time >= saveTime)
                        {
                            ...
                            if (Settings.UseSQL)
                            {
                                ...
                                if (MikeSource)
                                {
                                    List<AccountInfo> temp = SMain.Envir.sql.GetNewAccounts();
                                    if (temp != null &&
                                        temp.Count > 0)
                                        for (int i = 0; i < temp.Count; i++)
                                        {
                                            AccountList.Add(temp[i]);
                                            SMain.Envir.sql.UpdateData(temp[i], false, true);
                                        }
                                    for (int i = 0; i < AccountList.Count; i++)
                                    {
                                        AccountInfo tempAc = AccountList[i];
                                        if (tempAc != null)
                                        {
                                            
                                            /*
                                            if (SMain.Envir.sql.CheckExist(tempAc))
                                                SMain.Envir.sql.UpdateData(tempAc);
                                            else
                                                SMain.Envir.sql.InsetData(tempAc);
                                            */
                                            //  Check if there's any donations pending to be added
                                            bool addedCredits = false;
                                            int[] tempResult = SMain.Envir.sql.CheckDonations(tempAc);
                                            //  -1 means there were no values assigned
                                            if (tempResult[0] != -1 &&
                                                tempResult[1] != -1)
                                            {
                                                addedCredits = true;
                                                //  Add the purchased credits to the account
                                                tempAc.Credit += (uint)tempResult[1];
                                                //  Update the table
                                                SMain.Envir.sql.UpdateDonation(tempResult[0]);
                                            }
                                            for (int x = 0; x < tempAc.Characters.Count; x++)
                                            {
                                                CharacterInfo tmp = tempAc.Characters[x];
                                                if (tmp == null)
                                                    continue;

                                                //  Can't be admin, dev, banned or deleted
                                                if (!tmp.AccountInfo.AdminAccount &&
                                                    !tmp.AccountInfo.DevAccount &&
                                                    !tmp.AccountInfo.Banned &&
                                                    !tmp.Banned &&
                                                    !tmp.Deleted)
                                                {
                                                    bool online = false;
                                                    ...
                                                    {
                                                        for (int y = 0; y < SMain.Envir.Players.Count; y++)
                                                        {
                                                            if (SMain.Envir.Players[y] == null)
                                                                continue;
                                                            if (SMain.Envir.Players[y].Name == tmp.Name)
                                                            {
                                                                online = true;
                                                                //  Check if the player had donated
                                                                if (addedCredits)
                                                                {
                                                                    //  Send a message to player telling them they have recieved the credits
                                                                    SMain.Envir.Players[y].ReceiveChat(string.Format("You have recieved {0} credits from your donation.", tempResult[1]), ChatType.System);
                                                                    //  Send the packet over to the client to update their balance.
                                                                    SMain.Envir.Players[y].Enqueue(new S.GainedCredit { Credit = (uint)tempResult[1] });
                                                                }
                                                            }
                                                        }
                                                    }
                                                    if (SMain.Envir.sql.CheckExist(tmp))
                                                        SMain.Envir.sql.UpdateData(tmp, online);
                                                    else
                                                        SMain.Envir.sql.InsetData(tmp, online);
                                                }
                                            }
                                        }
                                    }
                        ...
                        if (Time >= updateHeroRanks)
                        {
                            updateHeroRanks = Time + Settings.Minute * 5;
                            if (HeroRankings.Count > 0)
                            {
                                HeroRankings.OrderByDescending(x => x.Level).ThenByDescending(x => x.Experience);
                            }
                        }
        ...
        public void Process()
        {
            ...
            if (Time >= questTime)
            {
                questTime = Time + Settings.Second;
                ProcessTimedQuests();
            ...
            if (Time >= viptime)
            {
                viptime = Time + Settings.Hour;
                for (int i = 0; i < Players.Count; i++) Players[i].ProcessVIP();
            }
            ...
            if (Time >= GuildTime)
            {
                ...
                {
                    if (g.VoteExpire != DateTime.MinValue && g.PendingGuild != null)
                    {
                        if (g.VoteExpire <= DateTime.Now)
                        {
                            g.SendMessage(string.Format("Voting has ended! ", ChatType.Guild));

                            if (g.ProVote >= 5 && g.ProVote > g.ConVote)
                            {
                                ...
                                if (g.WarringGuilds.Contains(g.PendingGuild))
                                {
                                    return;
                                }

                                if (g.GoToWar(g.PendingGuild))
                                {
                                    g.SendMessage(string.Format("War started with {0}.", g.PendingGuild.PendingGuild), ChatType.Event);
                                    g.PendingGuild.SendMessage(string.Format("{0} has started a war", g.Name), ChatType.Event);
                                }
                            }

                            g.PendingGuild = null;
                            g.VoteExpire = DateTime.MinValue;
                        }
                    }
                }
        ...
        public static string exportInfo = @".\ExportInfo\";
        public string exportInfoFile = "Info.txt";
        public void SaveDB()
        {
            if (!Directory.Exists(Settings.DBPath))
                Directory.CreateDirectory(Settings.DBPath);
            using (FileStream stream = File.Create(Settings.DBPath + "Version.db"))
            {
            ...
            using (FileStream stream = File.Create(Settings.DBPath + "Map.db"))
            {
                using (BinaryWriter writer = new BinaryWriter(stream))
                {
            ...
            using (FileStream stream = File.Create(Settings.DBPath + "Item.db"))
            {
                using (BinaryWriter writer = new BinaryWriter(stream))
                {
            ...
            using (FileStream stream = File.Create(Settings.DBPath + "Mob.db"))
            {
                using (BinaryWriter writer = new BinaryWriter(stream))
                {
            ...
            using (FileStream stream = File.Create(Settings.DBPath + "NPC.db"))
            {
                using (BinaryWriter writer = new BinaryWriter(stream))
                {
            ...
            using (FileStream stream = File.Create(Settings.DBPath + "Quest.db"))
            {
                using (BinaryWriter writer = new BinaryWriter(stream))
                {
            ...
            using (FileStream stream = File.Create(Settings.DBPath + "Dragon.db"))
            {
                using (BinaryWriter writer = new BinaryWriter(stream))
                {
            ...
            using (FileStream stream = File.Create(Settings.DBPath + "Magic.db"))
            {
                using (BinaryWriter writer = new BinaryWriter(stream))
                {
            ...
            using (FileStream stream = File.Create(Settings.DBPath + "GameShop.db"))
            {
                using (BinaryWriter writer = new BinaryWriter(stream))
                {
            ...
            using (FileStream stream = File.Create(Settings.DBPath + "Conquest.db"))
            {
                using (BinaryWriter writer = new BinaryWriter(stream))
                {
            ...
            using (FileStream stream = File.Create(Settings.DBPath + "RespawnTick.db"))
            {
                using (BinaryWriter writer = new BinaryWriter(stream))
                {
            ...
            using (FileStream stream = File.Create(Settings.DBPath + "RecipeShop.db"))
            {
                using (BinaryWriter writer = new BinaryWriter(stream))
                {
                    writer.Write(RecipeIndex);
                    writer.Write(RecipeShopContents.Count);
                    //  Save each content
                    for (int i = 0; i < RecipeShopContents.Count; i++)
                        RecipeShopContents[i].Save(writer);
                }
            }
            using (FileStream stream = File.Create(Settings.DBPath + "Crafting.db"))
            {
                using (BinaryWriter writer = new BinaryWriter(stream))
                {
                    writer.Write(CraftIndex);
                    writer.Write(CraftItems.Count);
                    for (int i = 0; i < CraftItems.Count; i++)
                        CraftItems[i].Save(writer);
                }
            }
            using (FileStream stream = File.Create(Settings.DBPath + "Raid.db"))
            {
                using (BinaryWriter writer = new BinaryWriter(stream))
                {
                    writer.Write(RaidIndex);
        ...
        public void BackupGuilds()
        {
            if (!Directory.Exists(Settings.GuildPath)) Directory.CreateDirectory(Settings.GuildPath);
            DirectoryInfo dir = new DirectoryInfo(Settings.GuildPath);
            if (!Directory.Exists(GuildDBPath))
                Directory.CreateDirectory(GuildDBPath);
            FileInfo[] files = dir.GetFiles();
            foreach(FileInfo file in files)
            {
                string temppath = Path.Combine(GuildDBPath, string.Format("{0} {1:00}-{2:00}-{3:00}-{4:00}-{5:00}-{6:0000}.bak", file.Name, Now.Hour, Now.Minute, Now.Second, Now.Day, Now.Month, Now.Year));
        ...
        public void LoadRobot()
        {
            ...
            string path = Path.Combine(Settings.RobotPath, Settings.RobotFileName + ".txt");
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                RobotInfo RobotCommand = RobotInfo.FromLine(lines[i]);

                if (RobotCommand == null)
                {
                    SMain.Enqueue(string.Format("Could not load Robot Line: {0}", lines[i]));
                    continue;
                }
        ...
        {
            foreach (RobotInfo R in RobotCommandList)
            {

                if (R._Day != (byte)Now.DayOfWeek && R._Day != -1) continue;
                if (R._Hour != Now.Hour && R._Hour != -1) continue;
                if (R._Min != Now.Minute && R._Min != -1) continue;

                // SMain.Enqueue(string.Format("Script: {0} - Day : {1} - Hour : {2} - Min : {3}", R.Command, R._Day, R._Hour, R._Min));


                CallRobotCommand(R.Command);

            }
        }


        private void CallRobotCommand(string command)
        {
            char[] delimiters = new[] { ',', '(', ')' };  // Get Params
            var CommandLines = command.Split(delimiters, StringSplitOptions.RemoveEmptyEntries);
            //Packet p;
            Map map;
            Point P;
            string[] read;
            ...
            if (CommandLines[0] == "") return;
            SMain.EnqueueDebugging(string.Format("Processing command : {0}", CommandLines[0]));
            switch (CommandLines[0].ToLower())
            {
                case "recallrandomlist":
                    #region RecallNameList
                    if (CommandLines.Length < 3) return;

                    fileName = Path.Combine(Settings.NameListPath, CommandLines[1] + ".txt");
                    if (!File.Exists(fileName))
                    {
                        SMain.EnqueueDebugging(string.Format("File does not exist : {0}", fileName));
                        return;
                    }
                    ...
                    int instance = 0;
                    int idx = -1;
                    for (int j = 0; j < MapInfoList.Count; j++)
                    {
                        if (MapInfoList[j].FileName.ToLower() == CommandLines[2] && idx == -1)
                            idx = MapInfoList[j].Index;
                    }
                    CommandLines[2] = CommandLines[2].Replace(" ", "");
                    if (CommandLines.Length == 4)
                    {
                        instance = Convert.ToInt32(CommandLines[3]);
                        if (idx != -1)
                            map = SMain.Envir.GetMap(idx);
                        else
                            map = SMain.Envir.GetMapByNameAndInstance(CommandLines[2], instance);
                    }
                    else if (idx != -1)
                        map = SMain.Envir.GetMap(idx);
                    else
                    ...
                    if (map == null)
                    {
                        SMain.EnqueueDebugging(string.Format("Map does not exist : {0}", CommandLines[2]));
                        return;
                    }
                    ...
                    {
                        PlayerObject ob = GetPlayer(v);
                        if (ob == null) continue;

                        ob.TeleportRandom(200, 0, map);
                ...
                case "recallramelist":
                    #region RecallNameList
                    if (CommandLines.Length < 5) return;
                    ...
                    map = SMain.Envir.GetMapByNameAndInstance(CommandLines[2], Convert.ToInt32(CommandLines[3]));

                    if (map == null) return;

                    P = new Point(Convert.ToInt32(CommandLines[4]), Convert.ToInt32(CommandLines[5]));
                ...
                case "mapflag":
                    #region MapFlag
                    if (CommandLines.Length < 5) return;
                    map = SMain.Envir.GetMapByNameAndInstance(CommandLines[1], Convert.ToInt32(CommandLines[2]));
                    if (map == null)
                    {
                        return;
                    }
                    switch (CommandLines[3])
                    {
                        case "NoFight":
                            if (CommandLines[4] == "true")
                                map.Info.NoFight = true;
                            else
                                map.Info.NoFight = false;
                            break;
                        ...
                        case "Fight":
                            if (CommandLines[4] == "true")
                                ...
                                map.Info.Fight = false;
                            break;

                        case "NoDropMonster":
                            if (CommandLines[4] == "true")
                                map.Info.NoDropMonster = true;
                            else
                                map.Info.NoDropMonster = false;
                            break;

                        case "NoDropPlayer":
                            if (CommandLines[4] == "true")
                                map.Info.NoDropPlayer = true;
                            else
                                map.Info.NoDropPlayer = false;
                            break;

                        case "NoRecall":
                            if (CommandLines[4] == "true")
                                map.Info.NoRecall = true;
                            else
                                map.Info.NoRecall = false;
                            break;

                        case "NoDrug":
                            if (CommandLines[4] == "true")
                                map.Info.NoDrug = true;
                            else
                                map.Info.NoDrug = false;
                            break;

                        case "SafeZone":
                            if (CommandLines[4] == "true")
                                map.Info.SafeZone = true;
                            else
                                map.Info.SafeZone = false;
                            break;
                    }
                ...
                #endregion             
                case "clearnamelist":
                    #region ClearNameList
                    if (CommandLines.Length < 2) return;
                ...
                case "clearnewbieguild":
                    #region ClearNewbieGuild

                    var newbieGuild = GetGuild(Settings.NewbieName);

                    if (newbieGuild == null) return;

                    newbieGuild.DeleteMembers();
                ...
                case "announcement":
                    ...
                    tempChatType = Convert.ToByte(CommandLines[2]);

                    if (!Enum.IsDefined(typeof(ChatType), tempChatType)) return;



                    for (i = 0; i < Players.Count; i++)
                        Players[i].ReceiveChat(CommandLines[1], (ChatType)tempChatType);
                ...
                case "spawn":
                    ...
                    {
                        int count = 0;
                        if (CommandLines.Length < 7) return;

                        P = new Point(Convert.ToInt32(CommandLines[3]), Convert.ToInt32(CommandLines[4]));
                        if (P == null)
                        {
                            return;
                        }

                        map = SMain.Envir.GetMapByNameAndInstance(CommandLines[2]);
                        if (map == null)
                        {
                            return;
                        }

                        MonsterInfo mInfo = GetMonsterInfo(CommandLines[1]);
                        if (mInfo == null)
                        {
                            return;
                        }

                        if (CommandLines[6].ToLower() == "true")
                        {
                            foreach (MapObject M in Objects)
                            {
                                if (M.CurrentMap != map || M.Race != ObjectType.Monster || M.Dead) continue;
                                if (!mInfo.Name.Contains(M.Name)) continue;

                                count++;
                            }

                            if (count >= Convert.ToInt32(CommandLines[5])) return;
                        }

                        for (i = 0; i < (Convert.ToInt32(CommandLines[5]) - count); i++)
                        {
        ...
        {
            if (!Directory.Exists(ConquestDBPath))
                Directory.CreateDirectory(ConquestDBPath);
            if (!Directory.Exists(CraftingDBPath))
                Directory.CreateDirectory(CraftingDBPath);
            if (!Directory.Exists(DragonDBPath))
                Directory.CreateDirectory(DragonDBPath);
            if (!Directory.Exists(GameShopDBPath))
                Directory.CreateDirectory(GameShopDBPath);
            if (!Directory.Exists(ItemDBPath))
                Directory.CreateDirectory(ItemDBPath);
            if (!Directory.Exists(MapDBPath))
                Directory.CreateDirectory(MapDBPath);
            if (!Directory.Exists(MobDBPath))
                Directory.CreateDirectory(MobDBPath);
            if (!Directory.Exists(NPCDBPath))
                Directory.CreateDirectory(NPCDBPath);
            if (!Directory.Exists(QuestDBPath))
                Directory.CreateDirectory(QuestDBPath);
            if (!Directory.Exists(RecipeShopDBPath))
                Directory.CreateDirectory(RecipeShopDBPath);
            if (!Directory.Exists(RespawnTickDBPath))
                Directory.CreateDirectory(RespawnTickDBPath);
            if (!Directory.Exists(RaidDBPath))
                Directory.CreateDirectory(RaidDBPath);
            if (!Directory.Exists(VersionDBPath))
                Directory.CreateDirectory(VersionDBPath);
            if (!Directory.Exists(MagicDBPath))
                Directory.CreateDirectory(MagicDBPath);

            if (Directory.Exists(Settings.DBPath))
            {
                DirectoryInfo dir = new DirectoryInfo(Settings.DBPath);
                if (dir != null)
                {
                    FileInfo[] files = dir.GetFiles();
                    for (int i = 0; i < files.Length; i++)
                        if (File.Exists(files[i].FullName))
                        {
                            ...
                            if (files[i].Name == "Conquest.db")
                                path = ConquestDBPath;
                            if (files[i].Name == "Crafting.db")
                                path = CraftingDBPath;
                            if (files[i].Name == "Dragon.db")
                                path = DragonDBPath;
                            if (files[i].Name == "GameShop.db")
                                path = GameShopDBPath;
                            if (files[i].Name == "Item.db")
                                path = ItemDBPath;
                            if (files[i].Name == "Magic.db")
                                path = MagicDBPath;
                            if (files[i].Name == "Map.db")
                                path = MapDBPath;
                            if (files[i].Name == "Mob.db")
                                path = MobDBPath;
                            if (files[i].Name == "NPC.db")
                                path = NPCDBPath;
                            if (files[i].Name == "Quest.db")
                                path = QuestDBPath;
                            if (files[i].Name == "RecipeShop.db")
                                path = RecipeShopDBPath;
                            if (files[i].Name == "RespawnTick.db")
                                path = RespawnTickDBPath;
                            if (files[i].Name == "Raid.db")
                                path = RaidDBPath;
                            if (files[i].Name == "Version.db")
                                path = VersionDBPath;
                            File.Copy(files[i].FullName, path + string.Format("{0} {1:00}-{2:00}-{3:00}-{4:00}-{5:00}-{6:0000}.bak", files[i].Name, Now.Hour, Now.Minute, Now.Second, Now.Day, Now.Month, Now.Year));
                        }
                }
            }
            /*
            if (File.Exists(DatabasePath))
                File.Copy(DatabasePath, BackUpPath + string.Format("Server.DB {0:0000}-{1:00}-{2:00} {3:00}-{4:00}-{5:00}.bak", Now.Year, Now.Month, Now.Day, Now.Hour, Now.Minute, Now.Second));
                */
        }
        ...
        public void BeginSaveAccounts()
        {
            ...
            using (MemoryStream mStream = new MemoryStream())
            {
                if (File.Exists(AccountPath))
                {
                    if (!Directory.Exists(AccountDBPath)) Directory.CreateDirectory(AccountDBPath);
                    string fileName = string.Format("Accounts {3:00}-{4:00}-{5:00} {2:00}-{1:00}-{0:0000}.bak", Now.Year, Now.Month, Now.Day, Now.Hour, Now.Minute, Now.Second);
                    if (File.Exists(Path.Combine(AccountDBPath, fileName))) File.Delete(Path.Combine(AccountDBPath, fileName));
                    File.Move(AccountPath, Path.Combine(AccountDBPath, fileName));
                }
        ...
        public int MapEventCount = 0;
        public void LoadDB()
        {
            lock (LoadLock)
            {

                if (!File.Exists(Settings.DBPath + "Version.db"))
                    SaveDB();

                using (FileStream stream = File.OpenRead(Settings.DBPath + "Version.db"))
                {
                    using (BinaryReader reader = new BinaryReader(stream))
                    {
                        ...
                        if (LoadVersion < 139)
                            CraftIndex = reader.ReadByte();
                        if (LoadVersion < 139)
                            MapEventIndex = reader.ReadInt32();
                    }
                }
                MapEventCount = 0;
                using(FileStream stream = File.OpenRead(Settings.DBPath + "Map.db"))
                {
                    using (BinaryReader reader = new BinaryReader(stream))
                    {
                        if (LoadVersion > 138)
                        ...
                        for (int i = 0; i < count; i++)
                        {
                            ...
                            if (MapInfoList[i].PublicEvents.Count > 0)
                            {
                                MapEventCount += MapInfoList[i].PublicEvents.Count;
                                if (MapEventIndex == 0)
                                    MapEventIndex = 1;
                            }
                        }
                ...
                using (FileStream stream = File.OpenRead(Settings.DBPath + "Item.db"))
                {
                    using (BinaryReader reader = new BinaryReader(stream))
                    {
                        if (LoadVersion > 138)
                ...
                using (FileStream stream = File.OpenRead(Settings.DBPath + "Mob.db"))
                {
                    using (BinaryReader reader = new BinaryReader(stream))
                    {
                        if (LoadVersion > 138)
                ...
                using (FileStream stream = File.OpenRead(Settings.DBPath + "NPC.db"))
                {
                    using (BinaryReader reader = new BinaryReader(stream))
                    {
                        if (LoadVersion > 138)
                ...
                using (FileStream stream = File.OpenRead(Settings.DBPath + "Quest.db"))
                {
                    using (BinaryReader reader = new BinaryReader(stream))
                    {
                        if (LoadVersion > 138)
                ...
                using (FileStream stream = File.OpenRead(Settings.DBPath + "Dragon.db"))
                {
                    using (BinaryReader reader = new BinaryReader(stream))
                    {
                        if (LoadVersion >= 11) DragonInfo = new DragonInfo(reader, LoadVersion);
                        else DragonInfo = new DragonInfo();
                    }
                }
                using (FileStream stream = File.OpenRead(Settings.DBPath + "Magic.db"))
                {
                    using (BinaryReader reader = new BinaryReader(stream))
                    {
                        int count = reader.ReadInt32();
                        for (int i = 0; i < count; i++)
                            ...
                            //MagicInfoList.Clear(); Db Clear                        
                        FillMagicInfoList();
                    }
                }
                using (FileStream stream = File.OpenRead(Settings.DBPath + "GameShop.db"))
                {
                    using (BinaryReader reader = new BinaryReader(stream))
                    {
                        if (LoadVersion > 138)
                ...
                using (FileStream stream = File.OpenRead(Settings.DBPath + "Conquest.db"))
                {
                    using (BinaryReader reader = new BinaryReader(stream))
                    {
                        if (LoadVersion > 138)
                ...
                using (FileStream stream = File.OpenRead(Settings.DBPath + "RespawnTick.db"))
                {
                    using (BinaryReader reader = new BinaryReader(stream))
                    {
                ...
                using (FileStream stream = File.OpenRead(Settings.DBPath + "RecipeShop.db"))
                {
                    using (BinaryReader reader = new BinaryReader(stream))
                    {
                        if (LoadVersion > 138)
                            RecipeIndex = reader.ReadInt32();
                        int count = reader.ReadInt32();
                        RecipeShopContents.Clear();
                        for (int i = 0; i < count; i++)
                            RecipeShopContents.Add(new RecipeShop(reader, LoadVersion));
                    }
                }
                using (FileStream stream = File.OpenRead(Settings.DBPath + "Crafting.db"))
                {
                    using (BinaryReader reader = new BinaryReader(stream))
                    {
                        if (LoadVersion > 138)
                        ...
                        for (int i = 0; i < count; i++)
                        {
                            CraftItems.Add(new CraftItem(reader, LoadVersion));
                            if (CraftItems[i].Recipie > CraftIndex)
                                CraftIndex = CraftItems[i].Recipie;
                        }
                    }
                }
                ...
                if (LoadVersion > 147)
                {
                    using (FileStream stream = File.OpenRead(Settings.DBPath + "Raid.db"))
                    {
                        using (BinaryReader reader = new BinaryReader(stream))
                        {
                            RaidIndex = reader.ReadInt32();
                            int count = reader.ReadInt32();
                            for (int i = 0; i < count; i++)
                            {
                                Raids.Add(new RaidMap_Info(reader));
            ...
            /*
            if (LoadCustomVersion > 0)
            {
                AI_Customiser tmp = new AI_Customiser();
                CustomAIs = tmp.LoadAI_Customiser();
            }
            */
            LoadLineMessages();
            /*
        ...
        {
            Settings.ReloadShieldUpgrades();
            SMain.Enqueue("Shield settings have been reloaded.");
        }
        ...
        public void LoadRuneMaterials()  //Ice 12/24
        {
            RuneDrops.Clear();

            string path = Path.Combine(Settings.DropPath, Settings.RuneDropFilename + ".txt");
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                if (drop == null)
                {
                    SMain.Enqueue(string.Format("Could not load Rune drop: {0}", lines[i]));
                    continue;
                }
        ...
        public void FortuneBoxWeapon()  //Ice 12/24
        {
            FortuneBoxWeaponDrops.Clear();

            string path = Path.Combine(Settings.DropPath, Settings.FortuneBoxWeaponFilename + ".txt");
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                if (drop == null)
                {
                    SMain.Enqueue(string.Format("Could not load FortuneBoxWeapon drop: {0}", lines[i]));
                    continue;
                }

                FortuneBoxWeaponDrops.Add(drop);
            }

            FortuneBoxWeaponDrops.Sort((drop1, drop2) =>
            {
                if (drop1.Chance > 0 && drop2.Chance == 0)
                    return 1;
                if (drop1.Chance == 0 && drop2.Chance > 0)
        ...
        public void FortuneBoxArmour()  //Ice 12/24
        {
            FortuneBoxArmourDrops.Clear();

            string path = Path.Combine(Settings.DropPath, Settings.FortuneBoxArmourFilename + ".txt");
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                if (drop == null)
                {
                    SMain.Enqueue(string.Format("Could not load FortuneBoxArmour drop: {0}", lines[i]));
                    continue;
                }

                FortuneBoxArmourDrops.Add(drop);
            }

            FortuneBoxArmourDrops.Sort((drop1, drop2) =>
            {
                if (drop1.Chance > 0 && drop2.Chance == 0)
                    return 1;
                if (drop1.Chance == 0 && drop2.Chance > 0)
        ...
        public void FortuneBoxAccessory()  //Ice 12/24
        {
            FortuneBoxAccessoryDrops.Clear();

            string path = Path.Combine(Settings.DropPath, Settings.FortuneBoxAccessoryFilename + ".txt");
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                if (drop == null)
                {
                    SMain.Enqueue(string.Format("Could not load FortuneBoxAccessory drop: {0}", lines[i]));
                    continue;
                }

                FortuneBoxAccessoryDrops.Add(drop);
            }

            FortuneBoxAccessoryDrops.Sort((drop1, drop2) =>
            {
                if (drop1.Chance > 0 && drop2.Chance == 0)
                    return 1;
                if (drop1.Chance == 0 && drop2.Chance > 0)
        ...
        public void FortuneBoxOrb()  //Ice 12/24
        {
            FortuneBoxOrbDrops.Clear();

            string path = Path.Combine(Settings.DropPath, Settings.FortuneBoxOrbFilename + ".txt");
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                if (drop == null)
                {
                    SMain.Enqueue(string.Format("Could not load FortuneBoxOrb drop: {0}", lines[i]));
                    continue;
                }
            ...
            FortuneBoxOrbDrops.Sort((drop1, drop2) =>
            {
                if (drop1.Chance > 0 && drop2.Chance == 0)
                    return 1;
                if (drop1.Chance == 0 && drop2.Chance > 0)
        ...
        {
            if (ManagerRunning || _managerThread != null) return;
            ManagerRunning = true;
        ...
        //This loads/writes fine...
        public void LoadItemInfoFile()
        {
            _DatFileInfos.Clear();
            ...
            else
            {
                using (FileStream stream = File.OpenRead(@".\ItemInfo.dat"))
                {
                    using (BinaryReader reader = new BinaryReader(stream))
                    {
                        _DatFileTime = DateTime.FromBinary(reader.ReadInt64());
                        int count = reader.ReadInt32();
                        for (int i = 0; i < count; i++)
                            _DatFileInfos.Add(new ItemInfo(reader, LoadVersion, LoadCustomVersion));
                    }
                }
            }
        }
        ...
        private void StartEnvir()
        {
            ...
            RecipeShopContents.Clear();
            MonsterCount = 0;
            CraftItems.Clear(); //  Don't want double
            Raids.Clear();            
            #region MySQL
            if (Settings.UseSQL)
                if (sql == null)
            ...
            for (int x = 0; x < Raids.Count; x++)
            {
                Map raidMap = GetMap(Raids[x].MapIndex);
                Map lobbyMap = GetMap(Raids[x].LobbyIndex);
                if (raidMap != null &&
                    lobbyMap != null)
                    raidMap.Raids.Add(new RaidMap(raidMap, lobbyMap) { Info = Raids[x] });
            }

            SMain.Enqueue(string.Format("{0} NPC's loaded.", NPCInfoList.Count));
            SMain.Enqueue(string.Format("Load Version {0} : Custom Version : {1} : Hero Version {2}", LoadVersion, LoadCustomVersion, HeroLoadVersion));
            SMain.Enqueue(string.Format("{0} Maps : {1} Guild Territorys Loaded", MapInfoList.Count, GTMapList.Count));
            SMain.Enqueue(string.Format("{0} Crafting Recipes loaded : {1} Crafting Scrolls loaded.", CraftItems.Count, RecipeShopContents.Count));
            SMain.Enqueue(string.Format("Quests loaded: {0} |  Hero quests loaded: {1}", QuestInfoList.Count(x => !x.HeroQuest), QuestInfoList.Count(x => x.HeroQuest)));
            SMain.Enqueue(string.Format("{0} Login Notices : {0} Line Notices loaded.", LoginNotice.Count, LineMessages.Count));
            SMain.Enqueue(string.Format("{0} MapEvents loaded.", MapEventCount));
            ...
            SMain.Enqueue(string.Format("{0} Start Items : {1} Items loaded.", StartItems.Count, ItemInfoList.Count));
            for (int i = 0; i < MonsterInfoList.Count; i++)
            {
                //if (i == 203)
                MonsterInfoList[i].LoadDrops();
                MonsterInfoList[i].LoadEliteDrops();
            }
            SMain.Enqueue("Normal | Elite Drops Loaded.");
            LoadFishingDrops();
            SMain.Enqueue(string.Format("{0} Fish Drops Loaded.", FishingDrops.Count));
            LoadAwakeningMaterials();
            SMain.Enqueue(string.Format("{0} Awaken Materials Loaded.", AwakeningDrops.Count));
            LoadStrongBoxDrops();
            SMain.Enqueue(string.Format("{0} StrongBox Drops Loaded.", StrongboxDrops.Count));
            LoadBlackStoneDrops();
            SMain.Enqueue(string.Format("{0} BlackStone Drops Loaded.", BlackstoneDrops.Count));
            LoadRuneMaterials();
            SMain.Enqueue(string.Format("{0} Rune Materials Loaded.", RuneDrops.Count));
            FortuneBoxWeapon();
            SMain.Enqueue(string.Format("{0} FortuneWeapon Loaded.", FortuneBoxWeaponDrops.Count));
            FortuneBoxArmour();
            SMain.Enqueue(string.Format("{0} FortuneArmour Loaded.", FortuneBoxArmourDrops.Count));
            FortuneBoxAccessory();
            SMain.Enqueue(string.Format("{0} FortuneAccessory Loaded.", FortuneBoxAccessoryDrops.Count));
            FortuneBoxOrb();
            SMain.Enqueue(string.Format("{0} FortuneOrb Loaded.", FortuneBoxOrbDrops.Count));


            LoadRobot();
            SMain.Enqueue("Robot Loaded");
        ...
        private void StartManagerNetwork()
        {
            ...
            {
                _ServerManager = new TcpListener(IPAddress.Parse(Settings.IPAddress), Settings.ServerManagerPort);
                if (_ServerManager == null)
                {
                    SMain.Enqueue("Manager Listen error");
                    return;
                }
                _ServerManager.Start();
                _ServerManager.BeginAcceptTcpClient(ManagerConnection, null);
            }
            SMain.Enqueue("Server Manager Started.");
        }
        ...
        private void StartNetwork()
        {
            ...
            LoadConquests();
            LoadGTInfo();
            SMain.Enqueue(string.Format("{0} Accounts : {1} Guilds : {2} Conquests  Loaded.", AccountList.Count, GuildList.Count, ConquestInfos.Count));

            _listener = new TcpListener(IPAddress.Parse(Settings.IPAddress), Settings.Port);
            if (_listener == null)
                SMain.Enqueue("TcpListener failure");
            _listener.Start();
            _listener.BeginAcceptTcpClient(Connection, null);

            if (StatusPortEnabled) // Status port
            {
                _StatusPort = new TcpListener(IPAddress.Parse(Settings.IPAddress), Settings.StatusConPort);
                _StatusPort.Start();
        ...
        private void StopManager()
        {
            ...
            SMain.Enqueue(string.Format("Server Manager Stopped"));
        }
        ...
        private void StopNetwork()
        {
            ...
            expire = Time + 5000;
            while(ServerManagers.Count != 0 && Stopwatch.ElapsedMilliseconds < expire)
            {
                Time = Stopwatch.ElapsedMilliseconds;
                for (int i = ServerManagers.Count - 1; i >= 0; i--)
                    ServerManagers[i].Process();
        ...
        private void ManagerConnection(IAsyncResult result)
        {
            if (_ServerManager.Server == null)
                return;
            if (!ManagerRunning || !_ServerManager.Server.IsBound) return;
            try
            {
                TcpClient tempTcpClient = _ServerManager.EndAcceptTcpClient(result);
                lock (ServerManagers)
                    ServerManagers.Add(new ServerManager(tempTcpClient));
            }
            catch (Exception ex)
            {
                SMain.Enqueue(ex);
            ...
            {
                while (ServerManagers.Count >= 2)
                    Thread.Sleep(1);
                if (ManagerRunning && _ServerManager.Server.IsBound)
                    _ServerManager.BeginAcceptTcpClient(ManagerConnection, null);
            }
        }
        ...
        public List<AccountInfo> MatchAccountsByCreationIP(string IP, bool match = false)
        {
            if (string.IsNullOrEmpty(IP)) return new List<AccountInfo>(AccountList);
            ...
            for (int i = 0; i < AccountList.Count; i++)
            {
                if (match)
                {
                    if (AccountList[i].CreationIP.Equals(IP, StringComparison.OrdinalIgnoreCase))
                        list.Add(AccountList[i]);
                }
                else
                {
                    if (AccountList[i].CreationIP.IndexOf(IP, StringComparison.OrdinalIgnoreCase) >= 0)
                        list.Add(AccountList[i]);
                }
            }
        ...
        public List<AccountInfo> MatchAccountsByLastIP(string IP, bool match = false)
        {
            if (string.IsNullOrEmpty(IP)) return new List<AccountInfo>(AccountList);
            ...
            for (int i = 0; i < AccountList.Count; i++)
            {
                if (match)
                {
                    if (AccountList[i].LastIP.Equals(IP, StringComparison.OrdinalIgnoreCase))
                        list.Add(AccountList[i]);
                }
                else
                {
                    if (AccountList[i].LastIP.IndexOf(IP, StringComparison.OrdinalIgnoreCase) >= 0)
                        list.Add(AccountList[i]);
                }
            }
        ...
        public void CreateRaidInfo()
        {
            Raids.Add(new RaidMap_Info { Index = ++RaidIndex, StartLocation = new Point(0, 0), BossAreas = new Point[] { new Point(0, 0), new Point(0, 0), new Point(0, 0) } });
        }
        ...
        public void CreateCraftRecipe()
        {
            byte count = (byte)CraftItems.Count;
            CraftItems.Add(new CraftItem { Recipie = ++count });
        }
        ...
        /// <summary>
        /// Create a new Recipie always increasing the shop contents Index
        /// </summary>
        public void CreateRecipieShopContent()
        {
            RecipeShopContents.Add(new RecipeShop { Index = ++RecipeIndex });
        }

        public void CreateCraftItem()
        {
            CraftItems.Add(new CraftItem { Recipie = ++CraftIndex });
        }
        ...
        {
            QuestInfoList.Add(new QuestInfo(false) { Index = ++QuestIndex });
        }

        public void CreateHeroQuestInfo()
        {
            QuestInfoList.Add(new QuestInfo(true) { Index = ++QuestIndex });
        }
        ...
        public void Remove(CraftItem info)
        {
            CraftItems.Remove(info);
            byte tmp = 1;
            for (int i = 0; i < CraftItems.Count; i++)
            {
                CraftItems[i].Recipie = tmp;
                tmp++;
            }
        ...
        public HeroObject GetHero(string name)
        {
            for (int i = 0; i < Players.Count; i++)
            {
                var p = Players[i];
                if (p.GainedHero && p.Hero != null)
                    if (String.Compare(p.Hero.HeroName, name, StringComparison.OrdinalIgnoreCase) == 0)
                        return p.Hero;
            }
        ...
        public AuctionHouseInfo GetAuction(ulong aIndex)
        {
            for (int i = 0; i < AHouseList.Count; i++)
                if (AHouseList[i].Index == aIndex)
                    return AHouseList[i];
            return null;
        }
        ...
        public void ProcessNewDay()
        {
            ...
            DirectoryInfo dir = new DirectoryInfo(Settings.NameListPath + @"Sw\");
            if (dir != null)
            {
                ...
                foreach (DirectoryInfo dirs in directories)
                {
                    ...
                    for (int i = 0; i < files.Length; i++)
                    {
                        File.WriteAllLines(files[i].FullName, new string[] { });
                    }
                }
            }
        }
        ...
        public void ProcessTimedQuests()
        {
            foreach (CharacterInfo c in CharacterList)
            {
                foreach(QuestProgressInfo q in c.CurrentQuests)
                {
                    if (q.EndTime == -1) continue;

                    if (q.EndTime == 0)
                    {
                        if (c.Player != null)
                        {
                            c.Player.ReceiveChat("Quest " + q.Info.Name + " failed!",ChatType.System);
                            c.Player.AbandonQuest(q.Index);
                            break;
                        }
                    }
        ...
        private void ClearDailyQuests(CharacterInfo info)
        {
            ...
            if (info.Player != null)
            {
                ...
                info.Player.GetClientCompletedQuests();
            }       
        }
